...
class ParsePageV2:
    ...
    def overview_parse(self, main_info):
        items = main_info[0].xpath('./table[@class="detail-info"]/tbody/tr/td')
        infos = {}
        for item in items:
            title, info = item.xpath('./span/text()')
            # print(title.strip(), info.strip())
            infos[title.strip().split('：')[0]] = info.strip()
        return infos
    ...
    def div_parse(self, items):
        title = items.xpath('./div[2]/table/tbody/tr/td[2]/text()')
        if title:
            title = title[0].strip()
    ...
    def parse_table(self, element):
        ...
        heads_info = element.xpath('./tbody/tr')
        if heads:
            ...
            item = {}
            for tr in heads_info:
                keys = tr.xpath('./th/text()')
                # print('key:',keys)
                values = tr.xpath('./td/text()')
                # print('value',values)
                for index, value in enumerate(values):
                    if value and keys:
                        item[keys[index].strip()] = value.strip()
            return item
